...
class StandardAppraisalLines(models.Model):
    ...
    def write(self, vals):
        for rec in self:
            if rec.standard_appraisal_employee_line.state == 'employee_confirmation' and (not 'accrued_greed' in vals or len(vals.items()) > 1):
                raise exceptions.AccessError(
                    _('You are only allowed to change accrued greed in this state!')
                )
        return super(StandardAppraisalLines, self).write(vals)
...
class CustomizeAppraisal(models.Model):
    ...
    def write(self, vals):
        for rec in self:
            if rec.customize_appraisal_line_id.related_manager_appraisal_complete.manager_appraisal_line_for_employee.state == 'employee_confirmation' and (not 'accrued_degree_id' in vals or len(vals.items()) > 1):
                raise exceptions.AccessError(
                    _('You are only allowed to change accrued greed in this state!')
                )
        return super(CustomizeAppraisal, self).write(vals)