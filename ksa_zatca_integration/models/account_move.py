...
class AccountMove(models.Model):
    ...
    disable_odoo_invoices = fields.Boolean(related="company_id.disable_odoo_invoices")
    ...
    def _get_zatca_invoice(self, is_pos=False):
        ...
        if not is_pos and not self.company_id.l10n_sa_show_a4:
    ...
    def default_get(self, fields_list):
        ...
        conf = self.journal_id.company_id or self.company_id or self.env.company.sudo()
    ...
    def create_xml_file(self, previous_hash=0, pos_refunded_order_id=0):
        ...
        if not is_tax_invoice and self.is_enterprise and self.disable_odoo_invoices:
        ...
        for record in self:
            ...
            if (conf.is_zatca
                    and ((not conf.is_self_billed and record.move_type in ['out_invoice', 'out_refund']) or
                         (conf.is_self_billed and record.move_type in ['out_invoice', 'out_refund', 'in_invoice',
                                                                       'in_refund']))
                ...
                if (record.move_type in ['in_invoice',
                                         'in_refund'] and record.l10n_is_self_billed_invoice) or record.move_type in [
    ...
    def is_enterprise(self):
        module = self.env['ir.module.module'].sudo().search([('name', '=', 'web_enterprise'),
                                                             ('state', '=', 'installed')], limit=1)
        return bool(module)

    def action_invoice_print(self):
        ret = super().action_invoice_print()
        if not self._context.get('via_account_invoice_send', False) or (
                not self.disable_odoo_invoices and (not self.mapped('is_zatca') == [True] or
                                                    not self.mapped('zatca_invoice') == [True])):
            return ret

        is_tax_invoice = 1 if self.mapped('l10n_sa_invoice_type') == ['Standard'] else 0
        if is_tax_invoice:
            report_template = self._get_zatca_invoice()[0]
        else:
